...
MBERT_LOCATIONS_JAR = join(Path(__file__).parent, 'mBERT-locs/mBERT-locations-1.1.0-SNAPSHOT-jar-with-dependencies.jar')
...
class MbertLocationsRequest:
    ...
    def _call_mbert(self, jdk_path: str, mbert_jar_path: str) -> bool:
        request = self.to_str()
        if request is None:
            log.error('Empty request {0}'.format(self.repo_path))
            return False
        cmd = "JAVA_HOME='" + jdk_path + "' " + join(jdk_path, 'bin',
                                                     'java') + " -jar " + mbert_jar_path + " " + request
        print("call mbert cmd ... {0}".format(cmd))
        output = shellCallTemplate(cmd)
        log.info(output)
        return isfile(self.locs_output_file)

    def call(self, jdk_path: str, mbert_jar_path: str = MBERT_LOCATIONS_JAR) -> str:
        ...
        if not self.has_locs_output():
            if not self._call_mbert(jdk_path, mbert_jar_path):
                self.on_exit('exit_call_mbert')
                return None
    ...
    @staticmethod
    def call_static(req, jdk_path: str, mbert_jar_path: str = MBERT_LOCATIONS_JAR):
        req.call(jdk_path, mbert_jar_path)
        return req


class RemoteMbertLocationsRequest(MbertLocationsRequest):
    def __init__(self, vcs_url: str, rev_id: str, *args, **kargs):
        super(RemoteMbertLocationsRequest, self).__init__(*args, **kargs)
    ...
    def on_exit(self, reason):
        super(RemoteMbertLocationsRequest, self).on_exit(reason)
